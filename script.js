...
function toggleBankDetails() {
    const isChecked = document.getElementById('includeBankDetails')?.checked;
    const formGroup = document.getElementById('bankDetailsFormGroup');
    if (formGroup) {
...
function updatePreview() {
    ...
    rows.forEach(row => {
        ...
        if (isCompanyGstRegistered && gstPercent > 0) {
            if (!taxByRate[gstPercent]) {
                taxByRate[gstPercent] = { taxable: 0, taxAmount: 0 };
            }
            taxByRate[gstPercent].taxable += taxableValue;
            taxByRate[gstPercent].taxAmount += taxAmount;
        }

        // Dynamic Height Heuristic (approximate pixels)
        // Description column width is ~180px in A4 layout, holding ~32 characters per line.
        let estHeight = 40; // Base row height (padding + borders)
        if (desc) {
            const manualLines = desc.split('\n').length;
            const charWrapLines = Math.ceil(desc.length / 32);
            const totalLines = Math.max(manualLines, charWrapLines);
            estHeight += totalLines * 18;
        }

        itemsData.push({ name, desc, hsn, qty, rate, disc, gstPercent, taxableValue, totalValue, estHeight });
    });
    ...
    // Available heights for items on pages:
    // Page 1 header + Bill To + Table headers take ~300px.
    // Full printable item area is ~580px.
    // Totals + Bank details + Signature footer take ~320px.
    const MAX_PAGE_ITEM_HEIGHT = 580; 
    const RESERVED_FOOTER_HEIGHT = 320; 
    const MAX_LAST_PAGE_ITEM_HEIGHT = MAX_PAGE_ITEM_HEIGHT - RESERVED_FOOTER_HEIGHT; // ~ 260px

    for (let i = 0; i < itemsData.length; i++) {
        const item = itemsData[i];
        const isLastItem = (i === itemsData.length - 1);
        const maxAllowed = isLastItem ? MAX_LAST_PAGE_ITEM_HEIGHT : MAX_PAGE_ITEM_HEIGHT;

        if (currentHeightUsed + item.estHeight > maxAllowed && currentPageItems.length > 0) {
    ...
    // Safety check: ensure last page has enough space for totals & footer
    while (pagesItems.length > 0) {
        const lastPage = pagesItems[pagesItems.length - 1];
        let lastPageItemsHeight = 0;
        lastPage.forEach(it => lastPageItemsHeight += it.estHeight);

        if (lastPageItemsHeight > MAX_LAST_PAGE_ITEM_HEIGHT && lastPage.length > 1) {
            const poppedItem = lastPage.pop();
            pagesItems.push([poppedItem]);
        } else {
            break;
        }
    }
    ...
    for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
        ...
        for (const [inputId, selector] of Object.entries(map)) {
            ...
            if (input && preview) {
                ...
                if (inputId === 'dueDate') {
                    const dueDateRow = page.querySelector('#p-dueDateRow');
                    if (dueDateRow) {
                        dueDateRow.style.display = input.value.trim() ? '' : 'none';
                    }
                }
        ...
        const isBankIncluded = document.getElementById('includeBankDetails')?.checked || false;
        ...
        const bHolder = document.getElementById('bankAccountHolder')?.value.trim() || '';
        ...
        // Totals, Bank Details & Footer visibility (Only on final page)
        const isLastPage = (pageIdx === totalPages - 1);
        const pdfTotals = page.querySelector('.pdf-totals');
        const pdfFooter = page.querySelector('.pdf-footer');
        const bankBlock = page.querySelector('#p-bankDetailsBlock');
        ...
        if (isLastPage) {
            ...
            if (bankBlock) {
                if (isBankIncluded) {
                    ...
                    const holderLabel = page.querySelector('#p-bankHolderLabel');
                    const holderVal = page.querySelector('#p-bankAccountHolder');
                    if (holderLabel && holderVal) {
                        if (bHolder) {
                            holderLabel.style.display = 'inline';
                            holderVal.style.display = 'inline';
                        } else {
                            holderLabel.style.display = 'none';
                            holderVal.style.display = 'none';
                        }
                    }
                } else {
            ...
            const taxContainer = page.querySelector('#p-tax-breakdown-container');
            if (taxContainer) {
                taxContainer.innerHTML = '';
                if (isCompanyGstRegistered) {
                    const sortedRates = Object.keys(taxByRate).sort((a, b) => Number(a) - Number(b));
                    
                    if (sortedRates.length > 0) {
                        sortedRates.forEach(rateStr => {
                            const rateNum = Number(rateStr);
                            const info = taxByRate[rateStr];
                            
                            if (isInterState) {
                                const row = document.createElement('div');
                                row.className = 'row';
                                row.innerHTML = `<span>IGST ${rateNum}%:</span><span>${formatCurrency(info.taxAmount)}</span>`;
                                taxContainer.appendChild(row);
                            } else {
                                const halfRate = (rateNum / 2).toString().replace(/\.0$/, '');
                                const halfTax = info.taxAmount / 2;

                                const cgstRow = document.createElement('div');
                                cgstRow.className = 'row';
                                cgstRow.innerHTML = `<span>CGST ${halfRate}%:</span><span>${formatCurrency(halfTax)}</span>`;
                                taxContainer.appendChild(cgstRow);

                                const sgstRow = document.createElement('div');
                                sgstRow.className = 'row';
                                sgstRow.innerHTML = `<span>SGST ${halfRate}%:</span><span>${formatCurrency(halfTax)}</span>`;
                                taxContainer.appendChild(sgstRow);
                            }
                        });
                    }
                }
